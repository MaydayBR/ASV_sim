...
def log_csv(step_idx, i, tx, ty, x, y, dx, dy, dist):
...
with open("run_log.csv", "w", newline="") as f:
    ...
    while i < len(waypoints):
        ...
        log_csv(step_idx, i, tx, ty, x, y, dx, dy, dist)
...
last_tx, last_ty = waypoints[-1]
t_s = time.time() - start_time
dist = 0.0  # we're done; distance is effectively 0
set_hud(t_s, step_idx, wp_reached, last_tx, last_ty, x, y, dist)

fig.canvas.draw()
fig.canvas.flush_events()
time.sleep(1.0)  # optional: so you can actually see 4/4
