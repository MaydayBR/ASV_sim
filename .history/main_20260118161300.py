...
def are_we_near_a_obstacle(dx, dy):
    #given the boats current position we must determine if we are near an obstacle
...
with open("run_log.csv", "w", newline="") as f:
    ...
    while i < len(waypoints):
        ...
        near_obstacle = are_we_near_a_obstacle(dx, dy)
        if not near_obstacle
            heading = update_heading(heading, desired_heading, max_turn_rate, dt)   #angle that it is allowed to point
        else:
            avoid_vector = compute_avoid_vector(x, y, obstacles, safety_margin)
            heading = update_heading_near_obstacle()
