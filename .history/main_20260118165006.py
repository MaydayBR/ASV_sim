...
obstacles = [(6.0, 2.0, 2.0), (8.0, 8.0, 1.9), (3.0, 7.0, 0.9)]
...
max_turn_rate_deg = 60.0                            # degrees per second (tune: 30..120)
...
def compute_avoid_vector(x, y, obstacles, safety_margin):
    """
    Returns an avoidance vector (ax, ay).
    If no obstacle is close, returns (0.0, 0.0).
    """
    ax_avoid = 0.0
    ay_avoid = 0.0
    ...
    for (ox, oy, r) in obstacles:
        dxo = x - ox
        dyo = y - oy
        d = math.sqrt(dxo*dxo + dyo*dyo)

        # How close is "too close"?
        influence_radius = r + safety_margin

        # If inside the influence zone, push away
        if d < influence_radius and d > 1e-6:
            # closeness goes from 0 (at boundary) to 1 (at center)
            closeness = (influence_radius - d) / influence_radius

            # unit vector away from obstacle center
            ux = dxo / d
            uy = dyo / d

            strength = closeness * c

            # push strength increases as you get closer
            ax_avoid += ux * closeness
            ay_avoid += uy * closeness
...
with open("run_log.csv", "w", newline="") as f:
    ...
    while i < len(waypoints):
        ...
        # Blend them:
        avoid_weight = 2.0   # tune: bigger = stronger avoidance
